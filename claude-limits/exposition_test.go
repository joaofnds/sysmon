...
func TestWriteMetrics(t *testing.T) {
	t.Run("writes each limit's used percent and reset time", func(t *testing.T) {
		...
		writeMetrics(&b, []limit{
			...
			{Kind: "weekly_scoped", Surface: "Claude Code", UsedPercent: 4, ResetsAt: time.Unix(1790553600, 0)},
		})

		want := "# HELP claude_limit_used_percent Share of the limit used so far, in percent.\n" +
			"# TYPE claude_limit_used_percent gauge\n" +
			`claude_limit_used_percent{limit="session",model="",surface=""} 18` + "\n" +
			`claude_limit_used_percent{limit="weekly_scoped",model="Fable",surface=""} 13` + "\n" +
			`claude_limit_used_percent{limit="weekly_scoped",model="",surface="Claude Code"} 4` + "\n" +
			...
			`claude_limit_reset_timestamp_seconds{limit="session",model="",surface=""} 1790259600` + "\n" +
			`claude_limit_reset_timestamp_seconds{limit="weekly_scoped",model="Fable",surface=""} 1790553600` + "\n" +
			`claude_limit_reset_timestamp_seconds{limit="weekly_scoped",model="",surface="Claude Code"} 1790553600` + "\n"
		if b.String() != want {
			t.Fatalf("got\n%s\nwant\n%s", b.String(), want)
		}
	})
